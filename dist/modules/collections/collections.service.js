...
let CollectionsService = class CollectionsService {
    ...
    async findAllByUser(id) {
        console.info(id);
        return await this.model.find({ "userId": new mongoose_2.Types.ObjectId(id) }).exec();
    }
    async findOne(id) {
        return await this.model.findOne({ "_id": id }).exec();
    }
    async create(user, collectionDTO) {
        collectionDTO.userId = new mongoose_2.Types.ObjectId(user.id);
